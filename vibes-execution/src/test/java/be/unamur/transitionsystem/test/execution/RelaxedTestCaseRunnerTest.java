...
public class RelaxedTestCaseRunnerTest {

    private static final Logger logger = LoggerFactory.getLogger(RelaxedTestCaseRunnerTest.class);
    ...
    public void testSimpleTestCase() throws Exception {
        ...
        ExecutionTree<Transition> exec = runner.run(lts,tc);
        logger.debug("Execution tree : {}", exec);
        assertNotNull(exec);
        ExecutionTree<Transition> expected = new ExecutionTree();
        ExecutionNode<Transition> node = expected.getRoot();
        node = node.addSon(new Transition(lts.getState("s0"), lts.getState("s1"), lts.getAction("act1")));
        node = node.addSon(new Transition(lts.getState("s1"), lts.getState("s2"), lts.getAction("act2")));
        node.addSon(new Transition(lts.getState("s2"), lts.getState("s0"), lts.getAction("act0")));
        assertThat(exec, equalTo(expected));
    }
